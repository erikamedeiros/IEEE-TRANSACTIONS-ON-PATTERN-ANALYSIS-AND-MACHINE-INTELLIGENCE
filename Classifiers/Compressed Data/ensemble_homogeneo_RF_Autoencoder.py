...
RF_model1 = RandomForestClassifier(n_estimators=10, random_state=0)
RF_model2 = RandomForestClassifier(n_estimators=50, random_state=0)
RF_model3 = RandomForestClassifier(n_estimators=100, random_state=0)
RF_model4 = RandomForestClassifier(n_estimators=500, random_state=0)
RF_model5 = RandomForestClassifier(n_estimators=1000, random_state=0)

estimators = []

estimators.append(('RF1', RF_model1))
estimators.append(('RF2', RF_model2))
estimators.append(('RF3', RF_model3))
estimators.append(('RF4', RF_model4))
estimators.append(('RF5', RF_model5))
...
for i in range(1, 31):
    ...
    X_train_encode = encoder.predict(x_treino)
    # encode the test data
    X_test_encode = encoder.predict(x_teste)
    
    #preparar o modelo para ser validado no kfold
    scores_ensemble =  evaluate_model(X_train_encode, y_treino, ensemble_model)
    results_ensemble.append(mean(scores_ensemble))
    ############################################################
      
    #Make predictions  
    ensemble_model.fit(X_train_encode, y_treino)
    predict_ensemble = ensemble_model.predict(X_test_encode)
    ...
    if maior_acuracia_treino_ensemble < aux_ensemble:
        ...
        x_teste_MC_ensemble = X_test_encode
